...
<div class="container_homepage">
    <h3 class="title">Tin nhắn bạn đã đọc</h3>
    <table class="table is-fullwidth main_content has-text-centered" id="tbl_data">
            <thead>
                ...
                <?php
                    if(isset($data["get_OldMessage"])){ 
                        $listOldMessage = json_decode($data["get_OldMessage"], true); 
                        $stt = 1;
                ?>
            <tbody>
                <?php
                    if(!empty($listOldMessage)){ 
                        foreach ($listOldMessage as $row) {
                            echo '<tr>
                                <td>'.$stt.'</td>
                                <td>'.$row['HoTen'].'</td>
                                <td>'.$row['SDT'].'</td>
